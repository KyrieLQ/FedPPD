...
def processing(args: argparse.ArgumentParser, splitted_data, processed_dir, client_id):
    if args.processing == "raw" or client_id is None:  # no processing or global data
        ...
        processed_data = random_feature_sparsity(args, splitted_data, processed_dir=processed_dir, client_id=client_id,
                                                 mask_prob=args.processing_percentage)
    elif args.processing == "random_feature_noise":
        from data.processing import random_feature_noise
        processed_data = random_feature_noise(args, splitted_data, processed_dir=processed_dir, client_id=client_id,
                                              noise_std=args.processing_percentage)
    elif args.processing == "random_edge_sparsity":
        from data.processing import random_edge_sparsity
        processed_data = random_edge_sparsity(args, splitted_data, processed_dir=processed_dir, client_id=client_id,
                                              mask_prob=args.processing_percentage)
    elif args.processing == "random_edge_noise":
        from data.processing import random_edge_noise
        processed_data = random_edge_noise(args, splitted_data, processed_dir=processed_dir, client_id=client_id,
        ...
        processed_data = random_label_sparsity(args, splitted_data, processed_dir=processed_dir, client_id=client_id,
                                               mask_prob=args.processing_percentage)
    elif args.processing == "random_label_noise":
        from data.processing import random_label_noise
        processed_data = random_label_noise(args, splitted_data, processed_dir=processed_dir, client_id=client_id,
                                            noise_prob=args.processing_percentage)
...
def random_feature_sparsity(args: argparse.ArgumentParser, splitted_data: dict, processed_dir: str, client_id: int,
                            mask_prob: float = 0.1):
    ...
    if args.task == "node_cls":
        ...
        masked_splitted_data["train_dataloader"] = DataLoader(
            [basedata for basedata in masked_splitted_data["data"][masked_splitted_data["train_mask"]]],
            batch_size=args.batch_size, shuffle=False)
        masked_splitted_data["val_dataloader"] = DataLoader(
            [basedata for basedata in masked_splitted_data["data"][masked_splitted_data["val_mask"]]],
            batch_size=args.batch_size, shuffle=False)
        masked_splitted_data["test_dataloader"] = DataLoader(
            [basedata for basedata in masked_splitted_data["data"][masked_splitted_data["test_mask"]]],
            batch_size=args.batch_size, shuffle=False)

    return masked_splitted_data


def random_feature_noise(args: argparse.ArgumentParser, splitted_data: dict, processed_dir: str, client_id: int,
                         noise_std: float = 0.1):
    ...
    if args.task == "node_cls":
        ...
        noised_splitted_data["train_dataloader"] = DataLoader(
            [basedata for basedata in noised_splitted_data["data"][noised_splitted_data["train_mask"]]],
            batch_size=args.batch_size, shuffle=False)
        noised_splitted_data["val_dataloader"] = DataLoader(
            [basedata for basedata in noised_splitted_data["data"][noised_splitted_data["val_mask"]]],
            batch_size=args.batch_size, shuffle=False)
        noised_splitted_data["test_dataloader"] = DataLoader(
            [basedata for basedata in noised_splitted_data["data"][noised_splitted_data["test_mask"]]],
            batch_size=args.batch_size, shuffle=False)
...
def random_label_sparsity(args: argparse.ArgumentParser, splitted_data: dict, processed_dir: str, client_id: int,
                          mask_prob=0.1):
...
def random_label_noise(args: argparse.ArgumentParser, splitted_data: dict, processed_dir: str, client_id: int,
...
def random_edge_sparsity(args: argparse.ArgumentParser, splitted_data: dict, processed_dir: str, client_id: int,
                         mask_prob: float = 0.1):
    masked_edge_index_filename = osp.join(processed_dir, "mask",
                                          f"random_edge_mask_{mask_prob:.2f}_client_{client_id}.pt")
...
def random_edge_noise(args, splitted_data: dict, processed_dir: str, client_id: int, noise_prob: float = 0.1):
    noised_edge_index_filename = osp.join(processed_dir, "noise",
                                          f"random_edge_noise_{noise_prob:.2f}_client_{client_id}.pt")
    ...
    if args.task == "node_cls":
        if osp.exists(noised_edge_index_filename):
            ...
            # add negative edges
            retained_edge_index, added_edge_index = add_random_edge(edge_index=edge_index, p=noise_prob,
