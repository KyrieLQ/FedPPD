...
class AdaFGLClient(BaseClient):
    ...
    def execute(self):
        ...
        if self.message_pool["round"] == config["num_rounds_vanilla"]:
            self.phase = 1
            self.adafgl_model = AdaFGLModel(prop_steps=config["prop_steps"], feat_dim=self.task.num_feats,
                                            hidden_dim=self.args.hid_dim, output_dim=self.task.num_global_classes,
                                            train_mask=self.task.train_mask, val_mask=self.task.val_mask,
                                            test_mask=self.task.test_mask, r=config["r"])
            ...
            self.adafgl_model.non_para_lp(subgraph=self.task.data, soft_label=soft_label, x=self.task.data.x,
                                          device=self.device)
            self.adafgl_model.preprocess(adj=self.task.data.adj)
            self.adafgl_model = self.adafgl_model.to(self.device)

            # create optimizer
            self.adafgl_optimizer = Adam(self.adafgl_model.parameters(), lr=config["adafgl_lr"],
                                         weight_decay=config["adafgl_weight_decay"])
    ...
    def adafgl_postprocess(self, loss_ce_fn=nn.CrossEntropyLoss()):
        ...
        loss_train3 = loss_ce_fn(local_message_propagation[self.task.train_mask],
                                 self.task.data.y[self.task.train_mask])
    ...
    def get_adafgl_override_evaluate(self):
        ...
        def override_evaluate(splitted_data=None, mute=False):
            ...
            with torch.no_grad():
                ...
                # hete eval
                local_ori_logits, local_smooth_logits, local_message_propagation = self.adafgl_model.hete_forward(
                    self.device)
            ...
            metric_train = compute_supervised_metrics(metrics=self.args.metrics,
                                                      ...
                                                      suffix="train")
            metric_val = compute_supervised_metrics(metrics=self.args.metrics, logits=logits[splitted_data["val_mask"]],
