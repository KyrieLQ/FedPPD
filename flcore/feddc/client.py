...
class FedDCClient(BaseClient):
    ...
    def get_custom_loss_fn(self):
        def custom_loss_fn(embedding, logits, label, mask):
            ...
            if self.message_pool["round"] != 0:
                ...
                for local_state, global_state, drift_param, update_param, avg_param in zip(self.task.model.parameters(),
                ...
                return task_loss + (config["feddc_alpha"] / 2) * loss_drift + (
                            1 / (self.args.lr * self.args.num_epochs)) * loss_grad
            else:
                return task_loss
    ...
    def execute(self):
        ...
        with torch.no_grad():
            for it, (update_param, local_state, global_state) in enumerate(
                    zip(self.last_update, self.task.model.parameters(), self.message_pool["server"]["weight"])):
