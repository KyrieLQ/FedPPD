...
class FedGTAClient(BaseClient):
    def __init__(self, args, client_id, data, data_dir, message_pool, device):
        ...
        self.train_label_onehot = F.one_hot(self.task.data.y[self.task.train_mask].view(-1),
                                            self.task.num_global_classes).to(torch.float).to(self.device)
        ...
        if f"personalized_{self.client_id}" in self.message_pool["server"]:
            with torch.no_grad():
                for (local_param, global_param) in zip(self.task.model.parameters(),
                                                       self.message_pool["server"][f"personalized_{self.client_id}"]):
    ...
    def fedgta_postprocess(self):
        ...
        lp_moment_v = compute_moment(x=output_raw, num_moments=config["num_moments"], dim="v",
                                     moment_type=config["moment_type"])
