...
class FedStarClient(BaseClient):
    def __init__(self, args, client_id, data, data_dir, message_pool, device):
        ...
        self.task.load_custom_model(DecoupledGIN(input_dim=self.task.num_feats, hid_dim=self.args.hid_dim,
                                                 output_dim=self.task.num_global_classes,
                                                 n_se=config["n_rw"] + config["n_dg"], num_layers=self.args.num_layers,
        ...
        self.task.train_dataloader = DataLoader([self.data[i] for i in tmp], batch_size=self.args.batch_size,
                                                shuffle=False)
        tmp = torch.nonzero(self.task.val_mask, as_tuple=True)[0]
        self.task.val_dataloader = DataLoader([self.data[i] for i in tmp], batch_size=self.args.batch_size,
                                              shuffle=False)
        tmp = torch.nonzero(self.task.test_mask, as_tuple=True)[0]
        self.task.test_dataloader = DataLoader([self.data[i] for i in tmp], batch_size=self.args.batch_size,
                                               shuffle=False)
