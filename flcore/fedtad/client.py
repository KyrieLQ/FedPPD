...
import os
from flcore.fedtad._utils import cal_topo_emb
from flcore.fedtad.fedtad_config import config
...
class FedTADClient(BaseClient):
    ...
    def fedtad_initialization(self):
        ...
        if os.path.exists(ckr_filename):
            ...
            data = self.task.data
            graph_emb = cal_topo_emb(edge_index=data.edge_index, num_nodes=self.task.num_samples,
                                     max_walk_length=config["max_walk_length"]).to(self.device)
