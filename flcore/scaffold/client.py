...
class ScaffoldClient(BaseClient):
    ...
    def step_preprocess(self):
        for p, local_control, global_control in zip(self.task.model.parameters(), self.local_control,
    ...
    def update_local_control(self):
        with torch.no_grad():
            for it, (local_state, global_state, global_control) in enumerate(
                    zip(self.task.model.parameters(), self.message_pool["server"]["weight"],
                        self.message_pool["server"]["global_control"])):
                self.local_control[it].data = self.local_control[it].data - global_control.data + (
                            global_state.data - local_state.data) / (self.args.num_epochs)
