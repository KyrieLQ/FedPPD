    ...
    def local_subgraph_train_val_test_split(self, local_subgraph, split, shuffle=True):
        ...
        for class_i in range(local_subgraph.num_global_classes):
            ...
            val_mask += idx_to_mask_tensor(
                class_i_node_list[int(train_ * num_class_i_nodes): int((train_ + val_) * num_class_i_nodes)], num_nodes)
            test_mask += idx_to_mask_tensor(class_i_node_list[
                                            int((train_ + val_) * num_class_i_nodes): min(num_class_i_nodes, int((
                                                                                                                             train_ + val_ + test_) * num_class_i_nodes))],
                                            num_nodes)
