...
class NodeClsTask(BaseTask):
    ...
    def evaluate(self, mute=False):
        ...
        metric_train = compute_supervised_metrics(metrics=self.args.metrics, logits=logits[self.train_mask],
                                                  labels=self.data.y[self.train_mask], suffix="train")
        metric_val = compute_supervised_metrics(metrics=self.args.metrics, logits=logits[self.val_mask],
                                                labels=self.data.y[self.val_mask], suffix="val")
        metric_test = compute_supervised_metrics(metrics=self.args.metrics, logits=logits[self.test_mask],
    ...
    def default_model(self):
        return load_node_cls_default_model(self.args, input_dim=self.num_feats, output_dim=self.num_classes,
    ...
    def num_global_classes(self):
        ...
        return torch.max(self.data.y).item() + 1
    ...
    def load_train_val_test_split(self):
        if self.client_id is None:  # server
            ...
            if osp.exists(train_path) and osp.exists(val_path) and osp.exists(test_path):
                ...
                train_mask, val_mask, test_mask = self.local_subgraph_train_val_test_split(self.data,
                                                                                           self.args.train_val_test)
    ...
    def local_subgraph_train_val_test_split(self, local_subgraph, split):
        ...
        for class_i in range(local_subgraph.num_global_classes):
            ...
            train_mask += idx_to_mask_tensor(mask_tensor_to_idx(class_i_node_mask)[:int(train_ * num_class_i_nodes)],
                                             num_nodes)
            val_mask += idx_to_mask_tensor(mask_tensor_to_idx(class_i_node_mask)[
                                           int(train_ * num_class_i_nodes): int((train_ + val_) * num_class_i_nodes)],
                                           num_nodes)
            test_mask += idx_to_mask_tensor(
                mask_tensor_to_idx(class_i_node_mask)[int((train_ + val_) * num_class_i_nodes):], num_nodes)
