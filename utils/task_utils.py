...
def load_node_edge_level_default_model(args, input_dim, output_dim, client_id=None):
    ...
    if model_name == "gcn":
        ...
        return GAT(input_dim=input_dim, hid_dim=args.hid_dim, output_dim=output_dim, num_layers=args.num_layers,
                   dropout=args.dropout)
    elif model_name == "graphsage":
        from model.graphsage import GraphSAGE
        return GraphSAGE(input_dim=input_dim, hid_dim=args.hid_dim, output_dim=output_dim, num_layers=args.num_layers,
                         dropout=args.dropout)
    elif model_name == "sgc":
        from model.sgc import SGC
        return SGC(input_dim=input_dim, hid_dim=args.hid_dim, output_dim=output_dim, num_layers=args.num_layers,
                   dropout=args.dropout)
    elif model_name == "gcn2":
        from model.gcn2 import GCN2
        return GCN2(input_dim=input_dim, hid_dim=args.hid_dim, output_dim=output_dim, num_layers=args.num_layers,
                    dropout=args.dropout)
    else:
        raise ValueError
